...
public class PoissonRequestRateSchedulerTest {

    @Rule
    public ExpectedException thrown = ExpectedException.none();

    @DataProvider
    public static Object[][] provideInvalidPoissonRequestRateScheduler() {
    ...
    @Test
    @UseDataProvider("provideInvalidPoissonRequestRateScheduler")
    public void invalidPoissonRequestRateScheduler(final double rate, final TimeUnit unit,
                                            final double rampup, final TimeUnit rampupUnit, final Class<Exception> expectedException) {
        this.thrown.expect(expectedException);
        new PoissonRequestRateScheduler(rate, unit, rampup, rampupUnit);
    }

    @DataProvider
    public static Object[][] providePoissonRequestsPerSecond() {
    ...
    @Test
    @UseDataProvider("providePoissonRequestsPerSecond")
    public void requestsPerSecond(final double rate, final TimeUnit unit,
        ...
        final PoissonRequestRateScheduler s =
                new PoissonRequestRateScheduler(1.0, TimeUnit.SECONDS, 0.0, TimeUnit.SECONDS);
