...
public class MultipartRequestSupplierTest {
  ...
  public void createMultipartRequestSupplierVirtualHostStyleTest() throws URISyntaxException {
    ...
    final URI uri = new URI(
        "http://" + this.vaultName + "." + this.hostName + ":8080/" + this.objectName + "?uploads");
    final long objectSize = 10;
    final MultipartRequestSupplier request = createRequestSupplier(true, this.vaultName,
        this.hostName, this.objectName, objectSize, null, 1, false);
  ...
  public void createMultipartRequestSupplierWithObjectURIRootVirtualHostStyleTest()
    ...
    final long objectSize = 10;
    final URI uri = new URI(
        "http://" + this.vaultName + "." + this.hostName + ":8080/" + objectName + "?uploads");
    final MultipartRequestSupplier request = createRequestSupplier(true, this.vaultName,
        this.hostName, objectName, objectSize, null, 1, false);
  ...
  public void createMultipartRequestSupplierPathStyleTest() throws URISyntaxException {
    ...
    final URI uri = new URI(
        "http://" + this.hostName + ":8080/" + this.vaultName + "/" + this.objectName + "?uploads");
    final long objectSize = 10;
    final MultipartRequestSupplier request = createRequestSupplier(false, this.vaultName,
        this.hostName, this.objectName, objectSize, null, 1, false);
  ...
  public void createMultipartRequestSupplierWithUriRootPathStyleTest() throws URISyntaxException {
    ...
    final long objectSize = 10;
    final MultipartRequestSupplier request = createRequestSupplier(false, this.vaultName,
        this.hostName, this.objectName, objectSize, this.uriRoot, 1, false);
  ...
  public void testSingleSessionWithEvenParts() throws URISyntaxException {
    ...
    final long expectedPart1Size = 5242880;
    final long expectedPart2Size = 5242880;
  ...
  public void testSingleSessionWithUnevenParts() throws URISyntaxException {
    final long objectSize = (long) (5242880 * 1.5);
    final long expectedPart1Size = 5242880;
    final long expectedPart2Size = 5242880 / 2;
    genericTwoPartMultipartSequence(objectSize, expectedPart1Size, expectedPart2Size);
  }
  ...
  public void testMultipleSessionsWithEvenParts() throws URISyntaxException {
    final int targetSessions = 2; // don't change this
    final long objectSize = 5242880 * 2;
    final long expectedPart1Size = 5242880;
    final long expectedPart2Size = 5242880;
  ...
  private MultipartRequestSupplier createRequestSupplier(final boolean virtualHost,
      final String vaultName, final String hostName, final String objectName, final long objectSize,
      final String uriRoot, final int targetSessions, final boolean trailingSlash) {
    ...
    final Function<Map<String, String>, Credential> credentials =
        MoreFunctions.forSupplier(credentialSupplier);
    ...
    final Function<Map<String, String>, Long> partSizes =
        MoreFunctions.forSupplier(partSizeSupplier);
    ...
    return new MultipartRequestSupplier(id, scheme, host, port, uriRoot, container,
        ...
        credentials, body, virtualHost, null, null, false);
  }

  private void genericMultipleSessions(final int targetSessions, final long objectSize,
      final long expectedPart1Size, final long expectedPart2Size) throws URISyntaxException {
    final URI uri = new URI(
        "http://" + this.vaultName + "." + this.hostName + ":8080/" + this.objectName + "?uploads");

    final String uploadIdA = "abc";
    final String uploadIdB = "xyz";
    final MultipartRequestSupplier requestSupplier = createRequestSupplier(true, this.vaultName,
        this.hostName, this.objectName, objectSize, null, targetSessions, false);

    final Request reqA0 = requestSupplier.get();
    ...
    final Response respMockA0 = mock(Response.class);
    when(respMockA0.getStatusCode()).thenReturn(200);
    when(respMockA0.getContext())
        .thenReturn(ImmutableMap.of(Context.X_OG_MULTIPART_UPLOAD_ID, "abcd"));
    when(respMockA0.headers()).thenReturn(ImmutableMap.of("ETag", "tag0"));

    final Request reqMockA0 = mock(Request.class);
    ...
    contextMap.put(Context.X_OG_MULTIPART_PART_SIZE, String.valueOf(this.partSize));
    ...
    final Request reqB0 = requestSupplier.get();
    ...
    final Response respMockB0 = mock(Response.class);
    when(respMockB0.getStatusCode()).thenReturn(200);
    when(respMockB0.getContext())
        .thenReturn(ImmutableMap.of(Context.X_OG_MULTIPART_UPLOAD_ID, "abcd"));
    when(respMockB0.headers()).thenReturn(ImmutableMap.of("ETag", "tag0"));

    final Request reqMockB0 = mock(Request.class);
    final Map<String, String> contextMapB = new HashMap<String, String>();
    contextMap.put(Context.X_OG_MULTIPART_BODY_DATA_TYPE, "ZEROES");
    contextMap.put(Context.X_OG_MULTIPART_CONTAINER, this.vaultName);
    contextMap.put(Context.X_OG_CONTAINER_SUFFIX, String.valueOf(-1));
    contextMap.put(Context.X_OG_OBJECT_NAME, this.objectName);
    ...
    final Request reqB1 = requestSupplier.get();
    final Map<String, String> reqContextB1 = reqB1.getContext();
    ...
    Assert.assertEquals(String.valueOf(objectSize),
    ...
    final Request reqB2 = requestSupplier.get();
    ...
    final Request reqB3 = requestSupplier.get();
    ...
    Assert.assertEquals(
        ...
        reqB3.getBody().getContent());
    Assert.assertEquals(String.valueOf(objectSize),
        reqB3.getContext().get(Context.X_OG_OBJECT_SIZE));

    // Respond to A INITIATE and complete sequence
    requestSupplier.update(Pair.of(reqMockA0, respMockA0));
    final Request reqA1 = requestSupplier.get();
    final Map<String, String> reqContext1 = reqA1.getContext();
    ...
    final Request reqA2 = requestSupplier.get();
    ...
    final Request reqA3 = requestSupplier.get();
    Assert.assertNotNull(reqA3);
    Assert.assertEquals("COMPLETE", reqA3.getContext().get(Context.X_OG_MULTIPART_REQUEST));
    Assert.assertEquals(
        "<CompleteMultipartUpload><Part><PartNumber>1</PartNumber><ETag>tag1</ETag></Part><Part><PartNumber>2</PartNumber><ETag>tag2</ETag></Part></CompleteMultipartUpload>",
        reqA3.getBody().getContent());
    Assert.assertEquals(String.valueOf(objectSize),
        reqA3.getContext().get(Context.X_OG_OBJECT_SIZE));
  }

  private void genericMultipartSessions(final int targetSessions) throws URISyntaxException {
    final URI uri = new URI(
        "http://" + this.vaultName + "." + this.hostName + ":8080/" + this.objectName + "?uploads");

    final MultipartRequestSupplier requestSupplier = createRequestSupplier(true, this.vaultName,
        this.hostName, this.objectName, 1, null, targetSessions, false);
  ...
  private void genericTwoPartMultipartSequence(final long objectSize, final long expectedPart1Size,
      final long expectedPart2Size) throws URISyntaxException {
    final URI uri = new URI(
        "http://" + this.vaultName + "." + this.hostName + ":8080/" + this.objectName + "?uploads");
    ...
    final Request req0 = requestSupplier.get();
    ...
    final Response respMock0 = mock(Response.class);
    when(respMock0.getStatusCode()).thenReturn(200);
    when(respMock0.getContext())
        .thenReturn(ImmutableMap.of(Context.X_OG_MULTIPART_UPLOAD_ID, "abcd"));
    when(respMock0.headers()).thenReturn(ImmutableMap.of("ETag", "tag0"));

    final Request reqMock0 = mock(Request.class);
    final Map<String, String> contextMap = new HashMap<String, String>();
    contextMap.put(Context.X_OG_MULTIPART_BODY_DATA_TYPE, "ZEROES");
    contextMap.put(Context.X_OG_MULTIPART_CONTAINER, this.vaultName);
    contextMap.put(Context.X_OG_CONTAINER_SUFFIX, String.valueOf(-1));
    ...
    final Request req1 = requestSupplier.get();
    final Map<String, String> reqContext1 = req1.getContext();
    ...
    final Request req2 = requestSupplier.get();
    ...
    final Request req3 = requestSupplier.get();
