...
_get_container_lifecycle_,  _put_container_protection_, _get_container_protection_ keys respectively.
...
request. By default, WriteCopy operation will select a random object name from objects which have already been written for source object.
...
and round robin selection configuration is supported. Also, a weighted choice of object configuration blocks can
be specified. The resulting object name will be of the form:
...
----
.Choices of object configurations
[source, json]
----
"object": {
  "selection": "roundrobin",
