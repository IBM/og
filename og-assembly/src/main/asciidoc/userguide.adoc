...
*50 requests per second* against a Cleversafe dsNet. The test should write
*3 million objects* and a mix of file sizes should be used. *80%* of written
files should be a *fixed 5MB*. *20%* of files should be a *uniform
distribution* between *8MB and 12MB*. Object manager object files should be
stored at */var/log/objects* with a prefix of *soh_objects*.
...
"object_manager": {
  "object_file_location": "/var/log/objects",
...
The authentication block itself is optional, and will default to anoynmous
