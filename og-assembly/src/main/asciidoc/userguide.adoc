...
Read operation has a property static_website_virtual_host_suffix that can be used for specifying host suffix for reading
objects with a website read operation. As long as the dsnet configuration allows it, an object can be read with website
read operation.
...
Support for Checksum algorithm and Checksum have been added in OG 2.0.0.
checksum_type property can be used to specify the checksum algorithm to use for the content integrity check.
checksum_type is supported for operations write, overwrite, write_copy, multipart_upload. checksum type can be one of sha1, sha256, crc32, crc32c, crc64nvme.
For multipart upload operation, only composite checksum type is supported. Each part will carry the specificied checksum header.

Object Tagging operations have been added in release 1.11.0. PUT Object Tags, Get Object Tags and Delete Object Tags
have been added. Object Tags can also be added with Write operation.

Object Version support has been added in release 1.12.0. _read_, _metadata_, _delete_ support object versioning.
_object_version_selection_ property is added to control the proportions of versioned and non versioned read.

_list_object_versions_ operation has been added in release 1.12.0. See <<Operation ListObjectVersions>>

_put_object_lock_retention_, _get_object_lock_retention_, _put_object_lock_legal_hold_, _get_object_lock_legal_hold_
are added in version 1.14.0.
...
_multi_delete_, _write_tags_, _get_tags_, _delete_tags_,_put_object_lock_retention_, _get_object_lock_retention_,
_put_object_lock_legal_hold_, _get_object_lock_legal_hold_ keys respectively.
...
==== Operation Object Version Selector ====
If the storage end point returns x-amz-version-id object version header, ObjectGenerator will track this in the object
file. Optionally, the tracked version can be used for _read_, _metadata_ and _delete_ operation.
Please note that object generator does not know the latest version. So, using delete operation without version may or may
not delete the correct object in the Object Generator object store. It is provided for convenience and using this will
cause inconsistencies in the local object management.

Example read operation with the object version:
----
  "read": {
    "weight": 100,
    "sse_c_source": false,
    "object_version_selection": {
      "versioned": 0.0,
      "non_versioned": 1.0
    }
  }
...
==== Operation Write with Object Tags
Here is a sample write operation config with the tags.
The Put Tags operation can be used to put tags on an existing object. See below for sample write_tags configuration.
The tags_configuration can specify multiple tags. A tag consists of key value pairs. The Tag key is constructed from
key_char and key_length which indicates the number of multiples of key_char to make the key. The Tag value is
constructed from value_char and value_length which indicates the number of multiples of value_char to make the value.
The key char and value char can be from allowed character set.
...
_list_object_versions_ operation will use randomly selected object from object file as prefix and key-marker queryparameters
 and if there is a version it will use that as version-id-marker query parameter.

All other properties can be configured as in _list_ operation. See <<Operation List configuration>>
...
----
==== Operation Put Tags Configuration ====
The Put Tags operation can be used to put tags on an existing object. See below for sample write_tags configuration.
The tags_configuration can specify multiple tags. A tag consists of key value pairs. The Tag key is constructed from
key_char and key_length which indicates the number of multiples of key_char to make the key. The Tag value is
constructed from value_char and value_length which indicates the number of multiples of value_char to make the value.
...
==== Operation Get Tags configuration ====
The Get Tags operation fetches the tags on an existing object.
----
  "get_tags": {
   "weight": 1.0
  }
...
==== Operation Delete Tags configuration ====
The Delete Tags operation deletes tags on an existing object.
...
The put object operation adds a retention configuration to the object.
retention property is required. If object_version_selection property is specified, version id will not be used in the
request.
Example:
----
  "put_object_lock_retention": {
    "weight": 1.0,
...
==== Operation get object retention ====
The get object retention operation gets the retention configuration of the object.
Example:
----
  "get_object_lock_retention": {
    "weight": 1.0,
     "object_version_selection": {
      "versioned": 0.5,
      "non_versioned": 0.5
...
The put object legal hold adds the legal status to the objects. The legal hold status (ON/OFF) can be specified with
 respective weights. The default value is {on: 1.0, off: 0}
The put object legal hold adds the legal status to the objects.
Example:
----
  "put_object_lock_legal_hold": {
    "weight": 1.0,
     "object_version_selection": {
      "versioned": 0.5,
      "non_versioned": 0.5
     },
     "object_legal_hold_status_selection": {
      "on": 1.0,
      "off": 1.0
    },
    "content_md5": true
...
==== Operation get object legal hold ====
The get object legal hold operation retrieves the legal hold status of the object.
Example:
----
  "get_object_lock_legal_hold": {
    "weight": 1.0,
    "object_version_selection": {
      "versioned": 0.5,
      "non_versioned": 0.5
...
==== Operation write select object ====
This operation can be used to upload an object from file so that the object can be queried with query select
object later.
The object name for this operation has a numeric suffix that is used to track the actual file that was used to upload the object. This mapping is persisted in selectObjectSuffix.json file. This filename can be overridden with select_object_suffix_file property in OG configuration file. The select object suffix file is stored by default at /var/log/og. The directory where this file is located can be configured with select_operations_config_location property in OG configuration.

Here is a sample configuration for this operation. The files can be specified with a selection configuration object.
----
  "write_select_object": {
    "weight": 1.0,
    "write_select_body_config": {
      "selection": "roundrobin",
        "choices": [
        {
        "choice": {
            "filepath": "/var/log/og/500KB_uncompressed.csv"
        },
         "weight": 1.0
        },
        {
        "choice": {
            "filepath": "/var/log/og/2MB_uncompressed.csv"
        },
         "weight": 1.0
        }
      ]
...
==== Operation query select object ====
This operation can be used to execute SelectObjectContent s3 API. The SQL query to be used in SelectObjectContent request body has to be stored in selectContent.json file. This filename can be overridden with select_content_query_file property in OG configuration file. This file is stored by default at /var/log/og. The directory where this file is located can be configured with select_operations_config_location property in OG configuration.

Here is a sample content of selectContent.json file.
----
[

  {
    "filepath":"/var/log/og/1KB.json",
    "selectbody": "<SelectObjectContentRequest xmlns=\"http://s3.amazonaws.com/doc/2006-03-01/\"> <Expression>SELECT UPPER(s.firstName), LOWER(s.lastName), CAST(s.salary as INT), LOWER(s.email), CHAR_LENGTH(s.department) FROM s3object s;</Expression><ExpressionType>SQL</ExpressionType><InputSerialization><CompressionType>NONE</CompressionType><JSON><Type>LINES</Type></JSON></InputSerialization><OutputSerialization><JSON/></OutputSerialization></SelectObjectContentRequest>"
  },
  {
    "filepath":"/var/log/og/2KB.json",
    "selectbody": "<SelectObjectContentRequest xmlns=\"http://s3.amazonaws.com/doc/2006-03-01/\"> <Expression>SELECT UPPER(s.firstName), LOWER(s.lastName), CAST(s.salary as INT), LOWER(s.email), CHAR_LENGTH(s.department) FROM s3object s;</Expression><ExpressionType>SQL</ExpressionType><InputSerialization><CompressionType>NONE</CompressionType><JSON><Type>LINES</Type></JSON></InputSerialization><OutputSerialization><JSON/></OutputSerialization></SelectObjectContentRequest>"
  }
]

----
Here is a sample configuration for query select object operation:
----
  "query_select_object": {
     "weight": 1.0
  }
----
It is important to note that when this operation configured, the object file that stores object should only have the objects that have been uploaded with write_select_object operation. Otherwise, OG cannot find a matching query and OG exits with error.

The object file can be filtered with selectable objects only with the object-file utility. Here is a sample invocation:
----
object-file --filter  --selectable-only false /var/log/og/object/bucket-s30.object -o filtered
----
This will filter the original object file bucket-s30.object and extract the select objects only in filtered0.object file. While running query_select_object operation, object_file_name property should be set to "filtered"
...
There are three types of supported concurrency in OG, *threads*, *ops*, and *poissonops*.
...
==== Poisson Request Concurrency
Poisson Request concurrency (poissonops) testing involves configuring a desired mean request
rate at which to send requests. Requests will be generated by a Poisson process with this
mean request rate. The time between which requests are generated is 1/count (in requests per second).
For example a "count" of 100 with "unit" in "seconds" will result in a mean time between requests of
10 milliseconds. Unlike Request Rate (ops) testing, which generates requests at fairly uniform times,
the time between events in Poisson Request Concurrency will be more variable,
although the mean time between requests should be approximately the same. A variable number of TCP
connections will be used to satisfy the configured rate. Poisson Request Rate testing is good
for measuring system latency. Note that Poisson Request concurrency works best at lower and medium
request rates (<= 1000 requests / second), but tends to be less useful at higher ops rates.

Request Rate testing supports _count_, _unit_, _rampup_ and _rampup_unit_
keys. Rampup allows an ops based test to build up to a steady request rate
over a configurable period of time.

.Poisson Request Rate (100 op/s, 10ms mean request interval) Example
[source, json]
----
"concurrency": {
  "type": "poissonops",
...
.Poisson Request Rate (500 op/minute, 2ms mean request interval) With Rampup Example
[source, json]
----
"concurrency": {
  "type": "poissonops",
...
The _shutdown_immediate_ is affected by _abort_mpu_when_stopping_. See <<mpu_abort>>.
...
==== Abort Multipart Upload transfers when stopping
The property _abort_mpu_when_stopping_ can be set to true for OG should send aborts for any Multipart uploads in progress
before shutting down. When this property is true, OG will wait for the ongoing part transfers to complete. It is possible
that the part transfers will fail if the corresponding Multipart upload transfer is aborted.

[[mpu_abort]] It should be noted that _abort_mpu_when_stopping_ and _shutdown_immediate_ properties cannot be true at the same time
in the configuration. If they are set true simultaneously, OG will exit with configuration error.
...
|abort_mpu_when_stopping
|Boolean
|No
|false

|select_operations_config_location
|String
|No
|/var/log/og/

|select_content_query_file
|String
|No
|selectContent.json

|select_object_suffix_file
|String
|No
|selectObjectSuffix.json
...
|static_website_virtual_host_suffix
|String
|No
|None
...
The credential file contains available accounts information in json format. The information depends on the API,
type of authentication used. Each json object can be in a single line or multiple lines.
...
Sample credentials for OpenStack API:
[source, json]
----
{"api": "openstack", "account_name": "osaccount101","basic_auth_password": "password", "project_name": "osaccount101", "basic_auth_username": "sos_user", "keystone_username": "keystone-user", "domain_name": "test-domain", "token": "9063fce56ff745d1b50ffbb46abbcbb6", "keystone_password": "password", "manager_username": "keystone-manager-user", "manager_password": "password", "containers": ["osc201", "osc202", "osc203"]}
----

Sample credentials for S3 API with AWS authentication:
[source, json]
----
{ "api": "s3", "account_name": "s3account1", "access_key": "jey3MxWQUE9YIaBUtXD1", "secret_key": "GDzvc103T8kM8bvZ5B8UYpFWB9QSJEoasaHfEuot", "containers": ["s3c7", "s3c8", "s3c9"]}
----

Sample credentials for S3 API with IAM authentication:
[source, json]
----
 {"api": "s3", "account_name": "storage_account", "iam_token": "Bearer eyJra...", "containers": ["iamc1", "iamc2", "iamc3"]}
----
